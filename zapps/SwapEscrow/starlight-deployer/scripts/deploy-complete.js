...
async function main() {
  ...
  functionNames.forEach(name => {
    const vkJson = JSON.parse(
      fs.readFileSync(`../orchestration/common/db/${name}_vk.key`, 'utf-8'),
    );
    if (vkJson.scheme) {
  ...
  const escrowShield = await EscrowShield.deploy(
    erc20Address,
    verifierAddress,
    vkInput,
  );
  ...
  console.log(
    'Approving from BankA Real Digital tokens to Escrow contract ...',
  );
  tx = await erc20.approve(escrowShieldAddress, 100000000);
  txReceipt = await tx.wait();
  if (txReceipt.status !== 1) {
